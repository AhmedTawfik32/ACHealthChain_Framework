...
function createChannel() {
  ...
  [[ $len -lt 10 ]] || [[ ! -d "organizations/peerOrganizations" ]] && bringUpNetwork="true" || echo "Network Running Already"
...
## Call the script to deploy a chaincode to the channel
function deployDiagnosisCC() {
  scripts/deployDiagnosisCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
## Call the script to deploy a chaincode to the channel
function deployPolicyCC() {
  scripts/deployPolicyCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
## Call the script to deploy a chaincode to the channel
function deployLogCC() {
  scripts/deployLogCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
if [ "$MODE" == "up" ]; then
  ...
  deployCC
elif [ "$MODE" == "deployDiagnosisCC" ]; then
  infoln "deploying diagnosis chaincode on channel '${CHANNEL_NAME}'"
  deployDiagnosisCC
elif [ "$MODE" == "deployPolicyCC" ]; then
  infoln "deploying policy chaincode on channel '${CHANNEL_NAME}'"
  deployPolicyCC
elif [ "$MODE" == "deployLogCC" ]; then
  infoln "deploying Log chaincode on channel '${CHANNEL_NAME}'"
  deployLogCC
